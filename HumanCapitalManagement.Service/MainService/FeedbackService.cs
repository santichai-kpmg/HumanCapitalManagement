...
namespace HumanCapitalManagement.Service.MainService
{
    public class FeedbackService : ServiceBase<Feedback>
    {
        ...
        public IEnumerable<Feedback> GetDataForSelect()
        {
            var sQuery = Query().Where(w => w.Active_Status == "Y");
            return sQuery.ToList();
        }

        public IEnumerable<Feedback> GetDataForSelectByRequest(string userid)
        {
            var sQuery = Query().Where(w => w.Active_Status == "Y" && w.Request_User == userid);
            return sQuery.ToList();
        }


        public IEnumerable<Feedback> GetDataForSelectByGiven(string userid)
        {
            var sQuery = Query().Where(w => w.Active_Status == "Y"  && w.Given_User == userid);
            return sQuery.ToList();
        }

        public Feedback FindForAddCadidate(int id)
        {
        ...
        public int CreateNew(ref Feedback s)
        {
            //s.Id = SelectMax();
        ...
        public int CreateNewOrUpdate(Feedback s)
        {
            var sResult = 0;
        ...
        public int AdminUpdate(Feedback s)
        {
            var sResult = 0;
