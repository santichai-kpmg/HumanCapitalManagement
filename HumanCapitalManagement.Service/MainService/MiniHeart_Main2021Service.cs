...
{
    public class MiniHeart_Main2021Service : ServiceBase<MiniHeart_Main2021>
    {
        public MiniHeart_Main2021Service(IRepository<MiniHeart_Main2021> repo) : base(repo)
        {
            //
        }
        public MiniHeart_Main2021 Find(int id)
        {
            return Query().SingleOrDefault(s => s.Id == id && s.Active_Status == "Y");
        }

        public MiniHeart_Main2021 FindByCondition(string Empno, int peroid)
        {
            return Query().SingleOrDefault(s => s.Emp_No == Empno && s.TM_MiniHeart_Peroid_Id == peroid && s.Active_Status == "Y");
        }


        public IEnumerable<MiniHeart_Main2021> GetDataForSelect()
        {
        ...
        public int CreateNew(ref MiniHeart_Main2021 s)
        {
            //s.Id = SelectMax();
        ...
        public int CreateNewOrUpdateMain(MiniHeart_Main2021 s)
        {
            var sResult = 0;
        ...
        public int CreateNewOrUpdate(MiniHeart_Main2021 s)
        {
            var sResult = 0;
            var _getData = Query().Where(w => w.Id == s.Id).FirstOrDefault();
            if (_getData == null)
        ...
        public int AdminUpdate(MiniHeart_Main2021 s)
        {
            var sResult = 0;
