...
{
    public class MiniHeart_Detail2021Service : ServiceBase<MiniHeart_Detail2021>
    {
        public MiniHeart_Detail2021Service(IRepository<MiniHeart_Detail2021> repo) : base(repo)
        {
            //
        }
        public MiniHeart_Detail2021 Find(int id)
        {
            return Query().SingleOrDefault(s => s.Id == id && s.Active_Status == "Y");
        }


        public IEnumerable<MiniHeart_Detail2021> FindByCondition(string Emp_No, int Main_Id)
        {
            return Query().Where(s => s.MiniHeart_Main_Id == Main_Id && s.Active_Status == "Y");
        }
        public IEnumerable<MiniHeart_Detail2021> ContReceiveByEmp_No(string Emp_No)
        {
        ...
        public IEnumerable<MiniHeart_Detail2021> GetReasonByEmpNo(string EmpNo, int PR_Id)
        {
            return Query().Where(s => s.Emp_No == EmpNo && s.MiniHeart_Main.TM_MiniHeart_Peroid_Id == PR_Id && s.Active_Status == "Y");
        }
        public IEnumerable<MiniHeart_Detail2021> GetReasonByEmpNoforcer(string EmpNo)
        {
            return Query().Where(s => s.Emp_No == EmpNo && s.Active_Status == "Y");
        }
        public IEnumerable<MiniHeart_Detail2021> GetDataForSelect()
        {
        ...
        public int CreateNew(ref MiniHeart_Detail2021 s)
        {
            //s.Id = SelectMax();
        ...
        public int CreateNewOrUpdate(MiniHeart_Detail2021 s)
        {
            var sResult = 0;
        ...
        public int AdminUpdate(MiniHeart_Detail2021 s)
        {
            var sResult = 0;
