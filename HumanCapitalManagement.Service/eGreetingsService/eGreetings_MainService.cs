...
{
    public class eGreetings_MainService : ServiceBase<eGreetings_Main>
    {
        public eGreetings_MainService(IRepository<eGreetings_Main> repo) : base(repo)
        {
            //
        }
        public eGreetings_Main Find(int id)
        {
            return Query().SingleOrDefault(s => s.Id == id && s.Active_Status == "Y");
        }

        public eGreetings_Main FindByCondition(string Empno, int peroid)
        {
            return Query().SingleOrDefault(s => s.Emp_No == Empno && s.TM_eGreetings_Peroid_Id == peroid && s.Active_Status == "Y");
        }


        public IEnumerable<eGreetings_Main> GetDataForSelect()
        {
        ...
        public int CreateNew(ref eGreetings_Main s)
        {
            //s.Id = SelectMax();
        ...
        public int CreateNewOrUpdateMain(eGreetings_Main s)
        {
            var sResult = 0;
        ...
        public int CreateNewOrUpdate(eGreetings_Main s)
        {
            var sResult = 0;
            var _getData = Query().Where(w => w.Id == s.Id).FirstOrDefault();
            if (_getData == null)
        ...
        public int AdminUpdate(eGreetings_Main s)
        {
            var sResult = 0;
