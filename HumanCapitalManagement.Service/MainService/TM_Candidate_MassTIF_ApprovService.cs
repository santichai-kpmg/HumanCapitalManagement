...
{
    public class TM_Candidate_MassTIF_ApprovService : ServiceBase<TM_Candidate_MassTIF_Approv>
    {
        public TM_Candidate_MassTIF_ApprovService(IRepository<TM_Candidate_MassTIF_Approv> repo) : base(repo)
        {
            //
        }
        public TM_Candidate_MassTIF_Approv Find(int id)
        {
            return Query().SingleOrDefault(s => s.Id == id);
        }
        public IEnumerable<TM_Candidate_MassTIF_Approv> GetTIFFormForApprove(string group_code, string[] agroup_code, string user_no, string sName, bool isAdmin = false)//,bool isAdmin
        {
            List<int> nStatus = new List<int>();
            nStatus.Add((int)StatusPR.Awaiting_Approval);
            nStatus.Add((int)StatusPR.Recruiting);

            var sQuery = Query().Where(w => w.active_status == "Y" && w.TM_Candidate_MassTIF.active_status == "Y" && w.TM_Candidate_MassTIF.submit_status == "Y"
                && w.TM_Candidate_MassTIF.TM_PR_Candidate_Mapping.active_status == "Y"
               // && nStatus.Contains(w.TM_Candidate_MassTIF.TM_PR_Candidate_Mapping.PersonnelRequest.TM_PR_Status.Id)
            ...
            if (!isAdmin)
            {
                ...
                {
                    sQuery = sQuery.Where(w => agroup_code.Contains(w.TM_Candidate_MassTIF.TM_PR_Candidate_Mapping.PersonnelRequest.TM_Divisions.division_code));
                }
                else
                {
            ...
            {
                sQuery = sQuery.Where(w => w.TM_Candidate_MassTIF.TM_PR_Candidate_Mapping.PersonnelRequest.TM_Divisions.division_code == group_code);
            }
            if (!string.IsNullOrEmpty(sName))
            {
                sQuery = sQuery.Where(w => ((w.TM_Candidate_MassTIF.TM_PR_Candidate_Mapping.TM_Candidates.first_name_en + "").Trim().ToLower()
        ...
        public int UpdateApprove(List<TM_Candidate_MassTIF_Approv> s, int nEmID, string UserUpdate, DateTime dNow)
        {
            var sResult = 0;
        ...
        public int CreateNewByList(List<TM_Candidate_MassTIF_Approv> s)
        {
            var sResult = 0;
            if (s != null && s.Any())
            {
                var _updateUser = s.Select(f => f.update_user).FirstOrDefault();
                List<TM_Candidate_MassTIF_Approv> lstEdit = new List<TM_Candidate_MassTIF_Approv>();
                foreach (var item in s.Where(w => w.TM_Candidate_MassTIF != null))
                {
                    var _checkDuplicate = Query().Where(w => w.Req_Approve_user == item.Req_Approve_user && w.TM_Candidate_MassTIF.Id == item.TM_Candidate_MassTIF.Id).FirstOrDefault();
                    if (_checkDuplicate == null)
                    {
                        int? nSeq = Query().Any(w => w.TM_Candidate_MassTIF.Id == item.TM_Candidate_MassTIF.Id) ? Query().Where(w => w.TM_Candidate_MassTIF.Id == item.TM_Candidate_MassTIF.Id).Max(m => m.seq) + 1 : 1;

                        item.seq = Query().Any(w => w.TM_Candidate_MassTIF.Id == item.TM_Candidate_MassTIF.Id) ? nSeq : item.seq;
                        Add(item);
                    }
        ...
        public int Update(TM_Candidate_MassTIF_Approv s)
        {
            var sResult = 0;
        ...
        public int RollBackStatus(int nTIFID, string UserUpdate, DateTime dNow)
        {
            ...
            if (nTIFID != 0)
            {
                var _getData = Query().Where(w => w.TM_Candidate_MassTIF.Id == nTIFID && w.active_status == "Y").ToList();
                if (_getData.Any())
                {
                    _getData.ForEach(ed =>
                    {
