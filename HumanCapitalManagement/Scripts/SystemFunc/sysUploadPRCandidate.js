...
/*==== Set Upload ====*/
function SetUploadPRCandidate(sID, sIMGTarget, hdf, Url) {
    ...
    $(document).ready(function () {
        ...
        $('#' + sID + '').on('fileselect', function (event, numFiles, label) {
            ...
            onUploadPRCandidate(sID, sIMGTarget, hdf, Url);
        });
    });

}
function onUploadPRCandidate(sID, sIMGTarget, hdf, Url) {
    $(function () {
                ...
                success: function (result) {
                    ...
                    if (sStatusresponse == SysProcess.SessionExpired) {
                        ...
                        if (lstData == undefined || lstData.length <= 0) {
                            ...
                            $('#gvwDataExcel').DataTable().clear().draw();
                            $('#gvwDataExcel').dataTable().fnDraw();
                            lstObj = [];
                            ...
                            $('#gvwDataExcel').DataTable().clear().draw();
                            $('#gvwDataExcel').dataTable().fnAddData(lstData);
                            $('#gvwDataExcel').dataTable().fnDraw();
                            lstObj = lstData;
                            $('#gvwDataExcel').DataTable().columns.adjust().draw()
                        }
...
function handleFilesPRCandidate(input) {
