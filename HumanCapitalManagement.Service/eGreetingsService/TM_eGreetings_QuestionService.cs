...
{
    public class TM_eGreetings_QuestionService : ServiceBase<TM_eGreetings_Question>
    {
        public TM_eGreetings_QuestionService(IRepository<TM_eGreetings_Question> repo) : base(repo)
        {
            //
        }
        public TM_eGreetings_Question Find(int id)
        {
            return Query().SingleOrDefault(s => s.Id == id && s.Active_Status == "Y");
        }
     
        public IEnumerable<TM_eGreetings_Question> GetDataForSelect()
        {
            var sQuery = Query().Where(w => w.Active_Status == "Y");
            return sQuery.ToList();
        }

        public IEnumerable<TM_eGreetings_Question> GetDataMainActive()
        {
            var sQuery = Query().Where(w => w.Active_Status == "Y" && w.TM_eGreetings_Group_Question.Active_Status == "Y");
            return sQuery.ToList();
        }

        #region Save Edit Delect 
        public int CreateNew(ref TM_eGreetings_Question s)
        {
            //s.Id = SelectMax();
        ...
        public int CreateNewOrUpdate(TM_eGreetings_Question s)
        {
            var sResult = 0;
        ...
        public int AdminUpdate(TM_eGreetings_Question s)
        {
            var sResult = 0;
