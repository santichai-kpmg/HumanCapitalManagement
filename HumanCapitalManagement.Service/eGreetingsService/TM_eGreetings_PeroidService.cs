...
{
    public class eGreetings_DetailService : ServiceBase<eGreetings_Detail>
    {
        public eGreetings_DetailService(IRepository<eGreetings_Detail> repo) : base(repo)
        {
            //
        }
        public eGreetings_Detail Find(int id)
        {
            return Query().SingleOrDefault(s => s.Id == id && s.Active_Status == "Y");
        }


        public IEnumerable<eGreetings_Detail> FindByCondition(string Emp_No, int Main_Id)
        {
            return Query().Where(s => s.eGreetings_Main_Id == Main_Id && s.Active_Status == "Y");
        }
        public IEnumerable<eGreetings_Detail> ContReceiveByEmp_No(string Emp_No)
        {
            return Query().Where(s => s.Emp_No == Emp_No && s.Active_Status == "Y");
        }
        public int ContReceiveByEmpNo(string EmpNo, int PR_Id)
        {
            return Query().Where(s => s.Emp_No == EmpNo && s.eGreetings_Main.TM_eGreetings_Peroid_Id == PR_Id && s.Active_Status == "Y").Count();
        }
        public IEnumerable<eGreetings_Detail> GetReasonByEmpNo(string EmpNo, int PR_Id)
        {
            return Query().Where(s => s.Emp_No == EmpNo && s.eGreetings_Main.TM_eGreetings_Peroid_Id == PR_Id && s.Active_Status == "Y");
        }
        public IEnumerable<eGreetings_Detail> GetReasonByEmpNoforcer(string EmpNo)
        {
            return Query().Where(s => s.Emp_No == EmpNo && s.Active_Status == "Y");
        }
        public IEnumerable<eGreetings_Detail> GetDataForSelect()
        {
        ...
        public int CreateNew(ref eGreetings_Detail s)
        {
            //s.Id = SelectMax();
        ...
        public int CreateNewOrUpdate(eGreetings_Detail s)
        {
            var sResult = 0;
        ...
        public int AdminUpdate(eGreetings_Detail s)
        {
            var sResult = 0;
