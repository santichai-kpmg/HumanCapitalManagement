...
{
    public class TM_Candidate_PInternService : ServiceBase<TM_Candidate_PIntern>
    {
        public TM_Candidate_PInternService(IRepository<TM_Candidate_PIntern> repo) : base(repo)
        {
            //
        }
        public TM_Candidate_PIntern Find(int id)
        {
            return Query().SingleOrDefault(s => s.Id == id);
        }
        public TM_Candidate_PIntern FindByMappingID(int id)
        {
        ...
        public TM_Candidate_PIntern FindByApproval(int id)
        {
            return Query().SingleOrDefault(s => s.Id == s.TM_Candidate_PIntern_Approv.FirstOrDefault(f => f.Id == id).TM_Candidate_PIntern.Id);
        }
        public IEnumerable<TM_Candidate_PIntern> FindByMappingArrayID(int[] id)
        {
        ...
        public IEnumerable<TM_Candidate_PIntern> GetDataForSelect()
        {
            var sQuery = Query().Where(w => w.active_status == "Y");
            return sQuery.ToList();
        }
        public IEnumerable<TM_Candidate_PIntern> GetSubGroupForSave(int[] aID)//,bool isAdmin
        {
            var sQuery = Query();
            if (aID != null)
            {
        ...
        public IEnumerable<TM_Candidate_PIntern> GetForAcknowledge(string PR_No, string Activities, string group_code, string[] agroup_code, string status, string sName, bool isAdmin = false, bool isHRAdmin = false)
        {
            List<int> nStatus = new List<int>();
            nStatus.Add((int)StatusPR.Awaiting_Approval);
            nStatus.Add((int)StatusPR.Recruiting);
            var sQuery = Query().Where(w => w.active_status == "Y" && w.TM_Candidate_PIntern_Approv.Where(w2 => w2.active_status == "Y" && string.IsNullOrEmpty(w2.Approve_status)).Count() <= 0
        ...
        public IEnumerable<TM_Candidate_PIntern> GetReportList(string PR_No, string Activities, string group_code, string[] agroup_code, string status,
             string ref_no,
            int supgroup_id,
            int position_id,
            string sName,
        ...
        public IEnumerable<TM_Candidate_PIntern> GetPInternByMappingID(int?[] aMID)
        {
        ...
        public int CreateNew(TM_Candidate_PIntern s)
        {
            //s.Id = SelectMax();
            Add(s);
        ...
        public int Update(TM_Candidate_PIntern s)
        {
            var sResult = 0;
        ...
        public int UpdateDate(TM_Candidate_PIntern s)
        {
            var sResult = 0;
            var _getDataPIntern = Query().Where(w => w.Id == s.Id).FirstOrDefault();
            if (_getDataPIntern != null)
        ...
        public int CreateNewOrUpdate(ref TM_Candidate_PIntern s)
        {
            var sResult = 0;
            var objSave = s;
        ...
        public int ApprovePInternForm(TM_Candidate_PIntern s)
        {
            var sResult = 0;
            var _getDataPIntern = Query().Where(w => w.Id == s.Id).FirstOrDefault();
            if (_getDataPIntern != null)
