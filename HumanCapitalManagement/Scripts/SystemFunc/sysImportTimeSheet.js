...
/*==== Set Upload ====*/
function SetImportTimeSheet(sID, sIMGTarget, hdf, Url) {
    ...
    $(document).ready(function () {
        ...
        $('#' + sID + '').on('fileselect', function (event, numFiles, label) {
            ...
            onImportTimeSheet(sID, sIMGTarget, hdf, Url);
        });
    });

}

function onImportTimeSheet(sID, sIMGTarget, hdf, Url) {
    $(function () {

        var fileUpload = $('#' + sID + '').get(0);
        var files = fileUpload.files;
        if (!files) {
            files = handleFilesPRTimeSheet(fileUpload);
        }
...
function handleFilesImportTimeSheet(input) {
